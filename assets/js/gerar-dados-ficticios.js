...
export function gerarDadosFicticios(totalDePessoas = 10, totalDeFichas = 2) {
    const fichas = [];
    const nomes = Array.from({ length: totalDePessoas }).map(() => {
    ...
    for (let i = 0; i < nomes.length; i++) {
        ...
        const datas = faker.date.betweens({
        ...
        for (let j = 0; j < totalDeFichas; j++) {
            const medicoes = Array.from({ length: nomes.length }).map(() => ({
...
//# sourceMappingURL=gerar-dados-ficticios.js.map