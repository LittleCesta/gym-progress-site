...
let fichaDados = gerarDadosFicticios(10, 5) ?? [];
const lista = document.querySelector(".lista-fichas");
...
//   renderizar()
// }
function selecionarFicha(element, nome) {
    if (!inputBox)
        return;
    inputBox.value = nome;
    if (!resultBox)
    ...
    for (const card of cards) {
        if (card.textContent && card.textContent.includes(nome)) {
            targetCard = card.parentElement; // .ficha-card
            break;
        }
    }
    // 📌 Rola até o card
...
inputBox.addEventListener("keyup", function (e) {
    ...
    const todasFichas = fichaDados.flat();
...
//# sourceMappingURL=get-fichas.js.map